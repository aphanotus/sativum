...
8. One more anole species, *Anolis sheplani*, needs its sequences added to the dataset. **Search** for the *ND2* sequence from this species in the NCBI database of nucleotide sequences using the `search.for.ncbi.ids` function as shown below.

```{text}
> new.id <- search.for.ncbi.ids("Anolis sheplani ND2")
```
...
The output of `search.for.ncbi.ids` is simply an GI index number for the NCBI database. To obtain the new sequences, we use the `fetch.sequence` function.

```{text}
> new.sequence <- fetch.sequence(new.id) 
```
...
This command saves the sequences in the `new.sequence` object and displays an abbreviated version of the sequence as output in the R console. 

9. **Combine** the new and old sequences together into a new dataset using the `combine.sequences` function.

```{text}
> (combined.ND2 <- combine.sequences(anole.ND2, new.sequence))
```
...
This command uses a simple trick in R. By surrounding the entire statement with parentheses it forces the contents of `combined.ND2` to be printed in the R console. (Otherwise the output from `combine.sequences` would not be displayed; just stored in the new object called `combined.ND2`.) Notice that the new sequence for *A. sheplani* is now below the original sequence for *A. valencienni*.
